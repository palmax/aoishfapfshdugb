...
WHERE ANIMAL.n_crotal = VACA.n_crotal AND
      VACA.n_crotal=PERTENECE.n_crotal AND
      RAZA.cod=PERTENECE.cod AND NOT EXISTS(SELECT PARE.n_crotal_madre
							   FROM PARE
...
CREATE VIEW VACA_RAZA AS 
SELECT ANIMAL.nombre, VACA.n_crotal, RAZA.nombre
FROM ANIMAL,VACA, pertenece, RAZA
WHERE VACA.n_crotal = ANIMAL.n_crotal AND pertenece.n_crotal= vaca.n_crotal and pertenece.Cod=raza.Cod;

CREATE VIEW SEMENTAL_RAZA AS 
SELECT ANIMAL.nombre AS animalito, SEMENTAL.n_crotal,RAZA.nombre as raza
FROM ANIMAL,SEMENTAL,pertenece,RAZA
WHERE SEMENTAL.n_crotal = ANIMAL.n_crotal AND SEMENTAL.n_crotal= pertenece.n_crotal AND pertenece.Cod=raza.cod;
